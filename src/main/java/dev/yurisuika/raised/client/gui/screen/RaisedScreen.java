...
public class RaisedScreen extends Screen {
    ...
    public void createPropertiesGrid() {
        x = new DoubleOption("options.raised.x", 0, client.getWindow().getScaledWidth() / 4, 1.0F, gameOptions -> (double)Option.getX(element), (gameOptions, value) -> Option.setX(element, value.intValue()), (gameOptions, option) -> {
            option.setTooltip(textRenderer.wrapLines(new TranslatableText("options.raised.x.tooltip"), 200));
            return option.get(gameOptions) == 0 ? ((OptionInvoker)option).invokeGetGenericLabel(ScreenTexts.OFF) : ((OptionInvoker)option).invokeGetGenericLabel(new LiteralText(Math.round(Math.ceil(((float)option.get(gameOptions) / ((float)option.getMax())) * 100)) + "%"));
        }).createButton(client.options, width - 110 - 16, 16, 110);
        y = new DoubleOption("options.raised.y", 0, client.getWindow().getScaledHeight() / 4, 1.0F, gameOptions -> (double)Option.getY(element), (gameOptions, value) -> Option.setY(element, value.intValue()), (gameOptions, option) -> {
            option.setTooltip(textRenderer.wrapLines(new TranslatableText("options.raised.y.tooltip"), 200));
            return option.get(gameOptions) == 0 ? ((OptionInvoker)option).invokeGetGenericLabel(ScreenTexts.OFF) : ((OptionInvoker)option).invokeGetGenericLabel(new LiteralText(Math.round(Math.ceil(((float)option.get(gameOptions) / ((float)option.getMax())) * 100)) + "%"));
        }).createButton(client.options, width - 110 - 16, 16 + 20 + 5, 110);
        position = new CyclingOption("options.raised.position", (gameOptions, integer) -> {
            ...
            cyclingOption.setTooltip(textRenderer.wrapLines(new TranslatableText("options.raised.position.tooltip"), 200));
            return ((OptionInvoker)cyclingOption).invokeGetGenericLabel(new TranslatableText(Option.getPosition(element).getTranslationKey()));
        }).createButton(client.options, width - 110 - 16, 16 + 20 + 5 + 20 + 5, 110);
        sync = new CyclingOption("options.raised.sync", (gameOptions, integer) -> {
            int id = Option.getSync(element).getId();
            Option.setSync(element, Sync.byId(id < Sync.values().length - 1 ? id + 1 : 0));
        }, (gameOptions, cyclingOption) -> {
            cyclingOption.setTooltip(textRenderer.wrapLines(new TranslatableText("options.raised.sync." + Option.getSync(element).asString() + ".tooltip", new TranslatableText(element.getTranslationKey())), 200));
            return ((OptionInvoker)cyclingOption).invokeGetGenericLabel(new TranslatableText(Option.getSync(element).getTranslationKey()));
        }).createButton(client.options, width - 110 - 16, 16 + 20 + 5 + 20 + 5 + 20 + 5, 110);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float delta) {
        ...
        switch (element) {
            ...
            case EFFECTS: {
                client.getTextureManager().bindTexture(new Identifier("textures/gui/container/inventory.png"));
                ...
                client.getTextureManager().bindTexture(new Identifier("textures/mob_effect/luck.png"));
                drawTexture(matrixStack,-91 + 1 + 3, offset - 24 - 1 + 3, 0, 0, 18, 18, 18, 18);
                RenderSystem.color4f(1.0F, 1.0F, 1.0F, percentY);
                client.getTextureManager().bindTexture(new Identifier("textures/mob_effect/unluck.png"));
            ...
            case PLAYERS: {
                ...
                client.getTextureManager().bindTexture(client.getSkinProvider().loadSkin(client.getSkinProvider().getTextures(profilePlayer).values().stream().findFirst().get(), MinecraftProfileTexture.Type.SKIN));
                ...
                client.getTextureManager().bindTexture(client.getSkinProvider().loadSkin(client.getSkinProvider().getTextures(profileNotch).values().stream().findFirst().get(), MinecraftProfileTexture.Type.SKIN));
            ...
            case TOASTS: {
                client.getTextureManager().bindTexture(new Identifier("textures/gui/toasts.png"));
        ...
        for (ClickableWidget widget : propertiesGrid) {
            if (widget != null && widget.isMouseOver(mouseX, mouseY)) {
                renderOrderedTooltip(matrixStack, ((OrderableTooltip)widget).getOrderedTooltip().orElse(null), mouseX, mouseY);
            }
        }
    }
