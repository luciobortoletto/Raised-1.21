        ...
        dispatcher.register(Commands.literal("raised")
                ...
                .then(Commands.literal("toggle")
                        .then(Commands.literal("texture")
                                .executes(context -> {
                                    context.getSource().sendSuccess(() -> Component.translatable("commands.raised.toggle.texture.query", Component.translatable(Option.getTexture().getSerializedName())), false);
                                    return 1;
                                })
                                .then(Commands.argument("texture", TextureArgument.texture())
                                        .executes(context -> {
                                            Option.setTexture(TextureArgument.getTexture(context, "texture"));
                                            context.getSource().sendSuccess(() -> Component.translatable("commands.raised.toggle.texture.set", Component.translatable(Option.getTexture().getSerializedName())), false);
                                            return 1;
                                        })
                                )
                        )
