    ...
    public static void register(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        dispatcher.register(literal("raised")
                ...
                .then(literal("toggle")
                        .then(literal("share")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.raised.toggle.share.query", config.toggle.share));
                                    return 1;
                                })
                                .then(argument("value", BoolArgumentType.bool())
                                        .executes(context -> {
                                            setShare(BoolArgumentType.getBool(context, "value"));
                                            context.getSource().sendFeedback(new TranslatableText("commands.raised.toggle.share.set", config.toggle.share));
                                            return 1;
                                        })
                                )
                        )
                        .then(literal("support")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.raised.toggle.support.query", config.toggle.support));
                                    return 1;
                                })
                                .then(argument("value", BoolArgumentType.bool())
                                        .executes(context -> {
                                            setSupport(BoolArgumentType.getBool(context, "value"));
                                            context.getSource().sendFeedback(new TranslatableText("commands.raised.toggle.support.set", config.toggle.support));
                                            return 1;
                                        })
                                )
                        )
